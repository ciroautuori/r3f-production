...
## Appendice: onBeforeCompile production pattern (mining cloudai, errata inclusa)

Iniezione nei materiali built-in senza riscrivere lo shader. Cheat-sheet degli injection point più usati: `begin_vertex`, `project_vertex`, `beginnormal_vertex`, `color_fragment`, `output_fragment`, `fog_fragment`.
...
material.onBeforeCompile = (shader) => {
  ...
  material.userData.shader = shader; // per aggiornare le uniform dal loop
};
material.customProgramCacheKey = () => 'mia-variante'; // OBBLIGATORIO: senza, collisioni di cache del programma
```

Tre regole che le fonti pubbliche non dicono: `customProgramCacheKey` è obbligatorio o three riusa il programma compilato di un altro materiale (shader nero dopo refactor); aggiorna le uniform via `material.userData.shader.uniforms.uProgress.value`, mai ricreando `onBeforeCompile`; `material.needsUpdate = true` solo quando cambia un define, mai per frame.

Dissolve con edge glow (snippet pronto, fragment):

```glsl
float noise = snoise(vUv * scale);
if (noise < uProgress) discard;
float edge = smoothstep(uProgress, uProgress + 0.1, noise);
gl_FragColor.rgb = mix(vec3(1.0, 0.4, 0.0) * 3.0, baseColor, step(0.5, edge)); // bordo incandescente HDR (supera 1.0 → bloom)
```

Debug: `renderer.debug.checkShaderErrors = true` in dev, mai in prod (costa). Chunk utili: `ShaderChunk` per `perspectiveDepthToViewZ` e simili invece di reimplementare la depth math.
