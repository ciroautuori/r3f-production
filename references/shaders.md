# Shaders — GLSL building blocks for award-level 3D
...
> ```glsl
> precision mediump float;   // see anti-patterns.md § 9 — non-negotiable for mobile
...
- **Patching a built-in material via `onBeforeCompile`** (shaders.md): three.js automatically provides both varyings. You just use them.
...
That material has built-in `chromaticAberration`. Don't reinvent. See shaders.md.
...
See `references/procedural-geometry.md` § #7 (fresnel rim) for a worked example.
...
- "I need a simple post effect" -> `ShaderPass` with a small fragment, see postprocessing-passes-api.md.
...
- `references/procedural-geometry.md` § #7 (Fresnel rim), #8 (UV scroll), #14 (Particle shader), #17 (Mesh morph), #30 (Background gradient mesh)
- `references/postprocessing-passes-api.md` for post-pipeline ShaderPass examples
- `references/anti-patterns.md` — don't forget `precision mediump float;` for raw ShaderMaterial

---

## Appendix: onBeforeCompile production pattern

Inject into built-in materials without rewriting the shader. Cheat-sheet of the most used injection points: `begin_vertex`, `project_vertex`, `beginnormal_vertex`, `color_fragment`, `output_fragment`, `fog_fragment`.
...
material.onBeforeCompile = (shader) => {
  ...
  material.userData.shader = shader; // to update uniforms from the loop
};
material.customProgramCacheKey = () => 'my-variant'; // MANDATORY: otherwise cache collisions of program
```

Three rules that public sources do not tell you: `customProgramCacheKey` is mandatory or three reuses the compiled program of another material (black shader after refactor); update uniforms via `material.userData.shader.uniforms.uProgress.value`, never recreating `onBeforeCompile`; `material.needsUpdate = true` only when a define changes, never per frame.

Dissolve with edge glow (ready snippet, fragment):

```glsl
float noise = snoise(vUv * scale);
if (noise < uProgress) discard;
float edge = smoothstep(uProgress, uProgress + 0.1, noise);
gl_FragColor.rgb = mix(vec3(1.0, 0.4, 0.0) * 3.0, baseColor, step(0.5, edge)); // HDR incandescent edge (exceeds 1.0 -> bloom)
```

Debug: `renderer.debug.checkShaderErrors = true` in dev, never in prod (cost). Useful chunks: `ShaderChunk` for `perspectiveDepthToViewZ` and similar instead of reimplementing the depth math.
