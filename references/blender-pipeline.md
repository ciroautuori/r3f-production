# Blender MCP recipes + GLB export
...
1. Four asset recipes for award-level-style hero objects
...
The classic award-level hero. Spinning coin, scroll-driven flip, gold material with HDRI reflections.
...
Pair with `MeshTransmissionMaterial` on the JS side (shaders.md). Blender role: produce the geometry; the material is overridden in three.
...
- Pair with `iridescence: 1.0` in three.js (shaders.md) for the soap-bubble look.
- Vertex count ≈ 642 — keep at this number to support Approach B vertex morph (shaders.md).
...
In three.js, register the KTX2 loader (shaders.md):
...
For a typical award-level site with ONE hero textured object, the encode time (5-10 minutes per texture) outweighs the size savings. Use compressed JPG/WebP unless you hit a real budget constraint.

---

## Shape keys → GLB morph targets

For shaders.md Approach A (mesh morphing).
...
// Animate via GSAP (shaders.md Approach A)
...
- `mesh.castShadow / receiveShadow` not set in three.js after load (shaders.md)
...
- `references/shaders.md` § #15 (GLB + Draco loader), #16 (KTX2), #17 (Mesh morphing)
