# Procedural geometry — code-only geometry generation

This skill's templates use AI-generated GLBs (via Blender MCP) as one path. **The other path is pure code** — primitives, math, vertex shaders, and Blender Python procedural. For 80–90% of award-level aesthetics, hand-coded geometry is **enough, faster, and more controllable** than reaching for AI.

This file documents the procedural toolkit. Cross-reference with `shaders.md` (which has runtime usage) and `blender-pipeline.md` (which has the Blender MCP recipes).
...
The full list, with award-level-relevant tweaks. Every one is one constructor call.
...
**award-level use:** Architectural elements (walls, blocks, sofa). Subdivisions for displacement.
...
**award-level use:** Abstract jewelry-like hero shapes. The `(p, q)` integers make different knots:
- `(2, 3)` — classic trefoil
- `(3, 4)` — pretzel
...
**award-level use:** Faceted geometric heroes. `detail: 0` looks gem-like; `detail: 4+` looks organic after smooth shading.
...
**award-level use:** Wireframe overlays (Active Theory aesthetic).

---

## 2. ExtrudeGeometry — 2D logo to 3D hero

award-level 3D-typography pattern in 30 lines:
...
**award-level use:** Hero objects that feel like "math made visible". Particularly good with iridescent or transmissive materials.
...
Combine with raycaster (shaders.md). Pass mouse worldPos as uniform; vertices near mouse spike outward:
...
For particle fields, debris, crowds, asteroid belts. shaders.md covers basics; here's an award-level-style use case:
...
`blender-pipeline.md` documents 4 base recipes. Here are advanced procedural patterns that go further.
...
**award-level use:** Hero objects with "carved" details. Combine with bevel for softness.
...
The **Recipe 3 (Blob)** in `blender-pipeline.md` is exactly this pattern — code generates shape, displace adds organic noise, smooth cleans it up. **No AI needed.**
...
└── NO → Is it a real-world specific object?
    ...
    └── NO → Can you sketch it from primitives + boolean + bevel?
        ├── YES → write Blender Python (blender-pipeline.md)
...
## 10. Cheat sheet — award-level hero objects, all from code

These are 80% of the hero objects on award-level Site of the Day. Each is one constructor + material:
...
Pair any of these with the canonical post chain (`references/postprocessing-passes-api.md`) and you have a publishable hero scene. **No AI, no marketplace, no licensing.**

---

## See also

- `references/procedural-geometry.md` § #13 (Instanced mesh), #14 (Particle shader), #17 (Mesh morph) — runtime patterns
- `references/shaders.md` — vertex/fragment shader building blocks
- `references/blender-pipeline.md` — Blender MCP recipes when you want to go beyond pure-code
- Every primitive here is used in a verified template; pair code geometry with the polish chain in `references/polish-chain.md`.
